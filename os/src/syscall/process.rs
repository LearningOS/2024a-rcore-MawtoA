...
use super::{SYSCALL_WAITPID, SYSCALL_YIELD};
...
pub fn sys_getpid() -> isize {
    ...
    add_syscall_count(SYSCALL_GETPID);
    current_task().unwrap().pid.0 as isize
}

pub fn sys_fork() -> isize {
    trace!("kernel:pid[{}] sys_fork", current_task().unwrap().pid.0);
    add_syscall_count(SYSCALL_FORK);
...
pub fn sys_exec(path: *const u8) -> isize {
    ...
    add_syscall_count(SYSCALL_EXEC);
...
pub fn sys_waitpid(pid: isize, exit_code_ptr: *mut i32) -> isize {
    ...
    add_syscall_count(SYSCALL_WAITPID);
    let task = current_task().unwrap();
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    add_syscall_count(SYSCALL_MMAP);
    if len != 0 {
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    add_syscall_count(SYSCALL_MUNMAP);
    if len != 0 {
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let elf_data = open_file(name.as_str(), OpenFlags::RDONLY);
    if let None = elf_data {
        return -1;
    }
    let elf_data = elf_data.unwrap().read_all();
